...
`hescope.core.paths.resolve_runtime_dir`. Nothing overrode either, so a test run
...
* `HESCOPE_DB_URL` -> a per-session sqlite file. This is the documented
  override (`hescope.store.db.get_engine`), so nothing in the package has to know.
* `hescope.core.paths.resolve_runtime_dir` -> a per-session directory. There is no
...
`hescope.store.db.DEFAULT_DB_URL` -- computed once, at import, from the real repo
